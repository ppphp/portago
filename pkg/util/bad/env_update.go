...
func env_update(makelinks int, target_root string, prev_mtimes=None, contents map[string][]string,
	env *config.Config, writemsg_level func(string, int, int), vardbapi *atom.vardbapi) {
	if vardbapi == nil {
...
func _env_update(makelinks int, target_root string, prev_mtimes map[string]int, contents map[string][]string, envC *config.Config,
	...
	util.EnsureDirs(envd_dir, -1, -1, 0755, -1, nil, true)
	fns := util.listdir(envd_dir, false, false, false, []string{}, true, true, false)
	sort.Strings(fns)
	templist := []string{}
	for _, x := range fns {
		if len(x) < 3 {
	...
	for _, x := range fns {
		...
		//try:
		myconfig := util.GetConfig(file_path, false, false, false, false, nil)
	...
	if len(oldld) != len(newld) {
		...
		if eq {
			myfd := util.NewAtomic_ofstream(ldsoconf_path, os.O_CREATE|os.O_RDWR|os.O_TRUNC, true)
	...
	if checksum.PrelinkCapable {
		...
		util.EnsureDirs(prelink_d, -1, -1, -1, -1, nil, true)
		newprelink := util.NewAtomic_ofstream(filepath.Join(prelink_d, "portage.conf"), os.O_RDWR|os.O_CREATE|os.O_TRUNC, true)
		...
		if strings.Split(string(f), "\n")[0] == "# prelink.conf autogenerated by env-update; make all changes to\\n" {
			...
			f := util.NewAtomic_ofstream(prelink_conf, os.O_RDWR|os.O_CREATE|os.O_TRUNC, true)
	...
	outfile := util.NewAtomic_ofstream(filepath.Join(eroot, "etc", "profile.env"), os.O_CREATE|os.O_RDWR|os.O_TRUNC, true)
	...
	outfile = util.NewAtomic_ofstream(filepath.Join(eroot, "etc", "csh.env"), os.O_CREATE|os.O_RDWR|os.O_TRUNC, true)
