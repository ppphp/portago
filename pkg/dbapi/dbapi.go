...
func (d *dbapi) _match_use(atom *dep.Atom, pkg *versions.PkgStr, metadata map[string]string, ignoreProfile bool) bool {
	...
	iUse := atom.NewIUse("", strings.Fields(metadata["IUSE"]), iUseImplicitMatch, useAliases, metadata["EAPI"])
...
func (b *bindbapi) writable() bool {
	if f, err := os.Stat(atom.firstExisting(b.bintree.pkgdir)); err != nil || f == nil {
		return false
	} else {
		return true
	}
...
func (b *BinaryTree) move_ent(mylist []string, repo_match func(string) bool) int {
	...
	for _, mycpv := range origmatches {
		...
		mytbz2 := xpak.NewTbz2(tbz2path)
		mydata := mytbz2.get_data()
		updated_items := atom.update_dbentries([][]string{mylist}, mydata, "", mycpv)
...
func (b *BinaryTree) _merge_pkgindex_header(src, dest map[string]string) {
	for _, i := range atom.iterIuseVars(src) {
		k := i[0]
		v := i[1]
		v_before := dest[k]
		if v_before != "" {
...
type portdbapi struct {
	*dbapi
	_use_mutable             bool
	repositories             *atom.repoConfigLoader
