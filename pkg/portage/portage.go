...
func (t *Tree) PortTree() *dbapi.PortageTree {
	if t._porttree != nil {
...
func (t *Tree) BinTree() *dbapi.BinaryTree {
	if t._bintree != nil {
...
func CreateTrees(config_root, target_root string, ts *TreesDict, env map[string]string, sysroot, eprefix string) *TreesDict {
	...
	if settings.ValueDict["ROOT"] == "/" && settings.ValueDict["EPREFIX"] == _const.EPREFIX {
		...
		settings = ebuild2.NewConfig(nil, nil, "", nil, "", "/", "/", "", false, clean_env, false, nil)
		settings.Lock()
	...
	for _, v := range myroots {
		...
		trees.valueDict[myroot].vartree = func() *dbapi.varTree {
			return dbapi.NewVarTree(mysettings.categories, mysettings)
		}
...
func Mtimedbfile() string {
	...
	*_mtimedbfile = path.Join(Settings().ValueDict["EROOT"], _const.CachePath, "mtimedb")
	return *_mtimedbfile
}
...
func update_dbentry(updateCmd []string, mycontent string, eapi string, parent *versions.PkgStr) string { // "", nil
	...
	if updateCmd[0] == "move" {
		...
		if strings.Contains(mycontent, oldValue) && dep.isValidAtom(newValue, false, false, false, "", false) {
			...
			for i, token := range splitContent {
				...
				atom, err := dep.NewAtom(token, nil, false, nil, nil, eapi, nil, nil)
				if err != nil {
					continue
				}
				if atom.cp != oldValue {
				...
				newAtom, _ := dep.NewAtom(strings.Replace(token, oldValue, newValue.value, 1), nil, false, nil, nil, eapi, nil, nil)
				if newAtom.Blocker != nil && parent != nil && parent.cp == newAtom.cp && len(dep.matchFromList(newAtom, []*versions.PkgStr{parent})) > 0 {
					continue
				}
...
func grab_updates(updpath string, prev_mtimes map[string]string) []struct{p string; s os.FileInfo; c string} {
	...
	for _, myfile := range mylist {
		...
		if len(update_data) > 0 || !myutil.Inmss(prev_mtimes, file_path) ||
	...
	for _, myline := range mylines {
		...
		if mysplit[0] == "move" {
			...
			for _, i := range []int{1, 2} {
				atom1, err := dep.NewAtom(mysplit[i], nil, false, nil, nil, "", nil, nil)
				if err != nil {
					//except InvalidAtom:
				} else {
		...
		if mysplit[0] == "slotmove" {
			...
			atom1, err := dep.NewAtom(pkg, nil, false, nil, nil, "", nil, nil)
			if err != nil {
				//except InvalidAtom:
			} else {
...
// nil, false
func update_config_files(config_root string, protect, protect_mask []string, update_iter map[string][][]*dep.Atom,
match_callback func(string,string,string)bool, case_insensitive bool) {
	...
	abs_user_config := filepath.Join(config_root, _const.UserConfigPath)
...
func _do_global_updates(trees *TreesDict, prev_mtimes map[string]string, quiet, if_mtime_changed bool) bool {
				...
				if !update_notice_printed {
					...
					if !quiet {
						msg.WriteMsgStdout(fmt.Sprintf("  %s='update pass'  %s='binary update'  "+
							...
							output.Bold("."), output.Bold("*"), output.Bold("#"), output.Bold("@"), output.Bold("s"), output.Bold("%"), output.Bold("S"), output.Bold("p")), 0)
					}
				}
	...
	for repo_name, myupd := range repo_map {
		...
		_world_repo_match := func(atoma, atomb *dep.Atom) bool {
			...
			if len(matches) > 0 && repo_match(vardb.aux_get(versions.Best(matches), map[string]bool{"repository": true}, "")[0]) {
				if len(portdb.match(atoma, 1)) != 0 {
					world_warnings[[2]*dep.Atom{atoma, atomb}] = true
				}
				return true
			} else {
				return false
